...
class ExaminationResultDetailPage extends StatefulWidget {
  ...
  const ExaminationResultDetailPage({super.key, required this.appointment});
...
class _ExaminationResultDetailPageState
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  widget.appointment.diagnosis?.split('.').first ??
  ...
  Widget _buildPrescriptionTicket(Map<String, dynamic> item) {
    String itemText(String key, [String fallback = '']) {
  ...
  Future<void> _handleShare(BuildContext context) async {
    try {
      ...
      buffer.writeln(
        'Ngày khám: ${DateFormat('dd/MM/yyyy').format(widget.appointment.appointmentDate)}',
      );
      buffer.writeln('');
      ...
      buffer.writeln(
        widget.appointment.notes ?? 'Theo dõi thêm sức khỏe tại nhà.',
      );
      buffer.writeln('-----------------------------------');
      buffer.writeln('Cài đặt Bệnh viện LPHV để quản lý sức khỏe thông minh!');
