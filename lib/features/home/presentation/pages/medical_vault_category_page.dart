...
class MedicalVaultCategoryPage extends StatefulWidget {
  ...
  State<MedicalVaultCategoryPage> createState() =>
      _MedicalVaultCategoryPageState();
}
...
class _MedicalVaultCategoryPageState extends State<MedicalVaultCategoryPage> {
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: uid == null
            ...
            : FirebaseFirestore.instance
                  ...
                  .where('status', whereIn: ['completed', 'waiting_payment'])
                  .snapshots(),
        builder: (context, snapshot) {
          ...
          if (widget.category == 'Sổ Xét Nghiệm')
            return _buildLabsList(appointments);
          if (widget.category == 'Đơn Thuốc')
            return _buildPrescriptionsList(appointments);
          if (widget.category == 'Chẩn Đoán HA')
            return _buildImagingGrid(appointments);
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final dateStr = DateFormat(
          'dd/MM/yyyy',
        ).format(lab['date'] as DateTime);
        final status = lab['status'] ?? 'Bình thường';
        final isWarning =
            status.toString().toLowerCase().contains('cao') ||
  ...
  Widget _buildPrescriptionsList(List<HospitalAppointmentModel> appointments) {
    final prescApps = appointments
        .where((a) => a.prescription != null && a.prescription!.isNotEmpty)
        .toList();
  ...
  List<HospitalAppointmentModel> _getMockAppointments() {
    return [
      HospitalAppointmentModel(
        ...
        notes:
            'Cần nghỉ ngơi nhiều hơn, uống thuốc đúng giờ và tái khám sau 10 ngày.',
        status: 'completed',
        paymentMethod: 'CASH',
        ...
        prescription: [
          {
            'name': 'Panadol Extra',
            'dosage': '1 viên/lần',
            'usage': 'Uống sau ăn (Sáng, Tối)',
            'quantity': '20 viên',
          },
          {
            'name': 'Hapacol 650',
            'dosage': '1 viên/lần',
            'usage': 'Khi đau đầu nhiều',
            'quantity': '10 viên',
          },
          {
            'name': 'Vitamin C 500mg',
            'dosage': '1 viên/ngày',
            'usage': 'Uống buổi sáng',
            'quantity': '30 viên',
          },
        ],
        labResults: [
      ...
      HospitalAppointmentModel(
        ...
        prescription: [
          ...
          },
          {
            'name': 'Alpha Choay',
            'dosage': '2 viên/lần',
            'usage': 'Uống hoặc ngậm (Ngày 3 lần)',
            'quantity': '30 viên',
          },
        ],
      ),
