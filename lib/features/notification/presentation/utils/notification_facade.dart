...
class NotificationFacade {
  ...
  static NotificationRepository get _repo =>
  ...
  static Future<void> onServiceResultSubmitted({
    ...
    await _repo.notify(
      NotificationEntity(
        ...
        body:
            'Kết quả $serviceName của bạn đã sẵn sàng. Vui lòng mở hồ sơ khám để xem chi tiết.',
  ...
  static Future<void> onExaminationCompleted({
    ...
    await _repo.notify(
      NotificationEntity(
        ...
        body:
            'Ca khám cùng BS. $doctorName đã hoàn tất. Vui lòng kiểm tra kết quả, hóa đơn hoặc đơn thuốc nếu có.',
        data: {
          'appointmentId': appointmentId,
          'doctorId': doctorId,
          'doctorName': doctorName,
  ...
  static Future<void> notifyDoctorTomorrowSchedule({
    ...
    await _repo.notify(
      NotificationEntity(
        ...
        body:
            'Ngày mai bạn có $appointmentCount ca khám. Vui lòng kiểm tra lịch trực.',
        data: {
  ...
  static Future<String?> _resolveEmail(String userId) async {
    try {
      ...
      return upper.data()?['email']?.toString() ??
